...
def main():
    ...
    for bid, logtxt in logs.items():
        ...
        if args.mode=="ai_try" and llm:
            ...
            try:
                out = llm.chat(build_llm_messages(ctx, fewshots=fewshots), temperature=float(cfg.get("temperature_report",0.0)))
                lines = sanitize_from_log(out, logtxt, span=args.span)
                if lines: report_lines = lines
